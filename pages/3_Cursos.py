...
def _course_card(course: dict):
    ...
    <div class="course-card">
        ...
        <div style="flex:1;">
            ...
            <div class="course-id">Código: {cid}</div>
            <span class="course-badge" style="background:rgba(69,123,157,0.2); color:{COLOR_INFO}; border:1px solid {COLOR_INFO};">
                🏢 {dept}
            </span>
            <span class="course-badge" style="background:rgba(155,93,229,0.15); color:{COLOR_PURPLE}; border:1px solid {COLOR_PURPLE};">
                💻 {modality}
            </span>
        </div>
    </div>""", unsafe_allow_html=True)
...
def _students_table(df: pd.DataFrame):
    ...
    for _, row in df.iterrows():
        ...
        rows_html += f"""
        <tr>
            <td style="font-family:monospace;">{row.get('student_id', '')}</td>
            <td>{row.get('Student Name', '')}</td>
            <td>{row.get('term', '')}</td>
            <td>
                <span class="status-pill" style="background:rgba(0,0,0,0.3); color:{color_nota}; border:1px solid {color_nota};">
                    {icon_nota} {nota:.1f}
                </span>
            </td>
            <td>
                <span style="color:{color_asist}; font-weight:600;">{asistencia:.1f}%</span>
            </td>
        </tr>"""
...
def show():
    ...
    <div style="background:linear-gradient(90deg,#457B9D,#9B5DE5); border-radius:12px; padding:20px 28px; margin-bottom:20px;">
        ...
        <p style="color:#dde; margin:4px 0 0 0; font-size:0.9rem;">
            Evalúa el desempeño general, asistencia y distribución de notas mediante filtros interactivos.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_filters:
        st.markdown("<div style='padding: 10px; background: rgba(255,255,255,0.03); border-radius:10px; border: 1px solid #2a2a4a;'>", unsafe_allow_html=True)
        st.markdown("<span style='font-size:0.85rem; color:#8892a4; text-transform:uppercase;'>Filtros de Análisis</span>", unsafe_allow_html=True)
    ...
    if df_filtered.empty:
        st.warning("⚠️ No hay estudiantes que coincidan con los filtros aplicados en este curso.")
        return
    ...
    with k1:
        _metric_card("Matriculados", str(total_alumnos), delta="Total filtrado", icon="👥")
    with k2:
    ...
    with k4:
        ...
        _metric_card("Asistencia Promedio", f"{asistencia:.1f}%" if pd.notna(asistencia) else "N/A", color=c_asist, icon="📅")
    ...
    with col_g1:
        # Histograma usando el mismo estilo del bar chart del estudiante
        fig_dist = px.histogram(
            df_filtered, x="final_grade", nbins=10,
            color_discrete_sequence=[COLOR_INFO],
            labels={'final_grade': 'Nota Final', 'count': 'Frecuencia'},
            template=PLOTLY_TPL
        )
        fig_dist.add_vline(
            x=11, line_dash="dash", line_color=COLOR_GOLD, line_width=2,
            annotation_text="Aprobación (11)", annotation_font=dict(color=COLOR_GOLD, size=11)
        )
        fig_dist.update_layout(
            title="Distribución de Calificaciones", title_font_size=15, height=360,
            plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
            margin=dict(t=50, b=40, l=20, r=20),
            xaxis=dict(range=[0, 21], title="Nota")
        )
        st.plotly_chart(fig_dist, use_container_width=True)

    with col_g2:
        # Scatter Plot con colores adaptados
        fig_scat = px.scatter(
            df_filtered, x="attendance_rate", y="final_grade",
        ...
        fig_scat.update_layout(
            title="Relación: Asistencia vs Rendimiento", title_font_size=15, height=360,
            plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
            xaxis_tickformat='%', margin=dict(t=50, b=40, l=20, r=20)
        )
    ...
    # Merge con data de estudiantes para obtener el nombre
    ...
    st.markdown(
        ...
        f"ID consultado: <code style='color:#8892a4'>{sel_course}</code></div>",
        unsafe_allow_html=True,
    )
